/* jenner-check bundle: %macro_variable_shutter_chance
   Source: shutter_chance/06_macros/macro_variable_shutter_chance.sas (macro definition, unmodified)
   Caller: adapted from the README's own "Usage Example" — nested %test macro that
   sets GLOBAL/LOCAL macro variables across two loop iterations and checkpoints the
   macro-variable environment via CheckID=A and CheckID=B. */
